...
async function getPidOnPort(port: number): Promise<number | null> {
	// Try ss (Linux)
	try {
		const result = await Bun.$`ss -tlnp sport = :${port}`.quiet();
		const match = result.stdout.toString().match(/pid=(\d+)/);
...
async function isChromiumDebugging(port: number): Promise<string | null> {
	try {
...
export default tool({
	...
	async execute(args) {
		...
		if (await isPortInUse(port)) {
			const browser = await isChromiumDebugging(port);
			if (browser) {
				return `Chromium is already running with remote debugging on port ${port} (${browser}).`;
			}
			return `Port ${port} is already in use by another process — cannot start Chromium debugging.`;
		}

		const proc = Bun.spawn(
			[CHROMIUM_BIN, `--remote-debugging-port=${port}`],
			{
				stdout: "ignore",
				stderr: "ignore",
			},
...
export const stop = tool({
	description:
		"Stop a running Chromium remote debugging instance by killing the process on the debugging port.",
	args: {
		port: tool.schema
			.number()
			.optional()
			.describe(
				`Remote debugging port to stop (default: ${DEFAULT_PORT})`,
			),
	},
	async execute(args) {
		const port = args.port ?? DEFAULT_PORT;
